...
1. **Bump the version in all seven markers — they must match** (drift ships a build
   that reports the wrong version). Find stragglers with
   `git grep -n "<previous version>"`:

   | Marker | What it drives |
   |---|---|
   | `infrastructure/version.py` → `__version__` | backend / FastAPI / `/health` |
   | `ui/src/config/constants.js` → `VERSION` | in-app UI marker |
   | `ui/package.json` → `version` | UI build |
   | `desktop/package.json` → `version` | Electron installer metadata |
   | `ui/public/landing.html` → hero badge | public marketing page |
   | `docs/brand/badge-release.svg` → 3 occurrences (`aria-label`, `<title>`, `<text>`) | README badge image |
   | `README.md` → badge `alt` text | accessible label for the above |

   The SVG's `textLength` is tuned to the string width, so it stays valid only
   while the version has the same character count. A move to a two-digit minor
   (`v1.10.0`) needs `textLength` widened too.

   **Do not** bump version strings inside `docs/product/COMPARISON.md` or
   `docs/product/VISION.md`. Those are *measurement records* — they state the
   version the claims were verified against, on a stated date. Re-stamping them
   without re-measuring turns a record into a false claim. They change when
   someone re-runs the measurement, not when someone cuts a tag.

   Both `package-lock.json` files are gitignored and regenerated by `npm install`,
   so they are not release markers — don't chase the version strings inside them.
2. Update the release notes. There is no `CHANGELOG.md` — notes live in the
   GitHub Release body for the tag.
