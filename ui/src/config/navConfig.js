...
//
// Two rules, so the menu reads as one set instead of a bag of parts. Both are
// checked by `npm run lint:ui` — they are not conventions, they are enforced.
//
// 1. `icon` — a name from components/ui/Icon.jsx. NEVER a character. The glyphs
...
//
// 2. `desc` — every surface and every tab has one, all 35, written in ONE voice:
//
//      · a NOUN PHRASE naming what the thing IS — "the routing skill graph",
//        not "browse the routing skill graph". Half of these used to be verbs
//        ("talk to your agents") and the surfaces were Title-Case imperatives
//        ("Monitor system health and reasoning"), so the menu spoke in three
//        voices at once and no two tiles scanned the same way.
//      · lower case (bar proper nouns), no full stop — it is a subtitle, not a
//        sentence.
//      · "and", never "&".
//      · short enough to sit on one line in a tile (≤ 34 chars).
export const SURFACES = [
  { id: "workspace", label: "Workspace", icon: "workspace", desc: "where you do the work", tabs: [
    ...
    { id: "project-state", label: "Project State", icon: "project",   desc: "what it knows about your repo", adv: true, group: "Plan" },
  ]},
  { id: "runs", label: "Runs", icon: "runs", desc: "every execution, inspected", adv: true, tabs: [
  ...
  { id: "cognition", label: "Cognition", icon: "cognition", desc: "system health and reasoning", adv: true, tabs: [
  ...
  { id: "memory", label: "Memory", icon: "memory", desc: "what Amagra knows", tabs: [
  ...
  { id: "research", label: "Analytics", icon: "analytics", desc: "how routing and memory behave", adv: true, tabs: [
  ...
  { id: "settings", label: "Setup", icon: "setup", desc: "how Amagra is set up", tabs: [
    ...
    { id: "model",    label: "Model",    icon: "model",    desc: "which model answers you" },
  ...
  { id: "system", label: "System", icon: "system", desc: "preferences and app info", tabs: [
    { id: "prefs",     label: "Settings",  icon: "settings",  desc: "how Amagra behaves" },
    { id: "shortcuts", label: "Shortcuts", icon: "shortcuts", desc: "every keyboard binding" },
    { id: "about",     label: "About",     icon: "about",     desc: "identity and engine state" },
  ]},
];
